...
Measured 9 Sep 2026 with the ColdFire port (`tools/ot_emu`), boot to the
...
- **`0x45d0dde0..0x46025de0`** — Octakit's window ("reserved recorder
  ...
  touches.
- **`0x47502c10..0x47fc7410`** — the stock **delay ring**, 10.8 MB, cleared
  through the uncached alias at boot instruction ~42 M, about 38 M
  instructions *after* the boot detour has returned, and live audio memory
  after that. ⚠️ An earlier reading of "8.8 MB free at 0x47700000" was
  made with a watch on cached addresses while the port mapped the alias as
  separate memory; it could not see this clear. **Retracted.** The port
  now folds the alias (`machine.h`, `alias()`), so the measurement is
  repeatable.
- **`0x47fc7410..0x47fe0000`** — the ~99 KB between the end of that clear
  and the 128 KiB Octakit keeps below the reset stack pointer. Never
  written in any phase measured. Octakit's stage (signature + packed
  runtime, 72,959 B) takes the first part when she is in the image;
  octabam's stage starts at `0x47fd9200` and its window at `0x47fdb000`,
  ceiling `0x47fe0000` — fixed addresses whatever the remix.
...
`tools/verify_dram_boot.py` (in `make verify`) boots the built image
...
- **Wipe-and-reload, generalised.** Octakit survives its project-load
  wipe with a hook that re-depacks from the stage. octabam's window needs
  none today (it is never written). Anything MB-sized will live in a
  wiped region and need the same hook; that mechanism is hers, and the
  one worth building together.
