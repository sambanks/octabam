...
Audio comes from the local DSP emulator (~6× real time, `docs/remixer/HARNESS.md`);
the ColdFire emulator (`docs/remixer/EMU.md`) provides the screens. Nothing in the
...
FX1's three `lea` references are repointed (`tools/build/build_fx1.py` proved that
...
core. See
`docs/remixer/MODULES.md` for the measured reason — it is `docs/firmware/DSP.md`'s "wrong
...
⚠️ **`tools/build/build_fx1.py`'s experiment never wrote FX1's cursor table** and
...
its own key. The classes are in `docs/remixer/MODULES.md`, with the measured reason. The same
...
(192/instance, `docs/firmware/CHIP.md`), so the row says what is missing rather than
...
never changes. The addresses are in `docs/firmware/DSP.md` §10 and belong there —
...
(`docs/firmware/DSP.md`, "the allocator's instance model"). Nothing is handed out
first-come.
...
Honest limits (`docs/remixer/EMU.md`): no audio and no key matrix in the emulator —
...
| effect | path |
|---|---|
| busverb | `tools/harness/render_reverb.py` — manifest slot → its positional param name (the two tables are proven slot-aligned by the selftest); MODE via `--mode`, wet via `--wet` |
