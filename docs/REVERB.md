...
| 1 | 3 | TONE | `r6+$3` | **LO + HI on one knob** (v8, 5 Sep 2026; was HP). 0..64 is the old LP 0..127 with HP off (dark → flat); 64..127 is the old HP 0..126 with LP wide open (flat → thin). **64 = the old HP 0 / LP 127 exactly** (render bit-identical; TONE 63 ≡ old LP 126 likewise). Merged so slot 4 could carry the host's send |
| 1 | 4 | -DEL | `r6+$4` | **this track's dry send into BusDelay** (v8; was LP). The 18 Aug 2026 send, back for the rig's "real send knobs": dry mono × knob, 3-bit headroom, into the DELAY accumulator; the delay counts the host as a client through a flag word (`Y:0x941`, the knob field, written every block) rather than a count RMW. Default 0 (phantom-client rule). ⚠️ A part saved before v8 loads its old LP=127 here = full send, and its HP=0 as TONE=0 = fully dark — `stamp-defaults` before play |
