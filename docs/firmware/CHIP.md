...
`tools/build/cycle_count.py` now subtracts bank growth and prints the live number.
...
| | cycles/sample | |
|---|---|---|
| `reverb_server` (BusVerb, with shimmer) | 758 | ✅ `tools/build/cycle_count.py` — **pre-8-line reading**; **~1 133** with the 8-line tank |
...
stalls modelled — so they are a floor. `tools/harness/dsp_host` **cannot** measure
...
every run; `tools/harness/rig_render.py` writes `meter.txt`), which is a second
...
| | ⚠️ That "ESAI setup" is real and the ESAIs really do carry audio — 8-slot network mode, verified 30 Aug 2026. `DSP.md` asserted the opposite for weeks while this line sat here; see `docs/firmware/EXTERNAL.md` §2. | |
...
| *(X data tables)* | the ColdFire uploads ~20 lookup tables to both DSPs at boot — waveforms, knob warps, coefficient banks. Catalogued in **`docs/firmware/TABLES.md`**; they cost us no placement and any effect may read them | Bryan T + verified here |
