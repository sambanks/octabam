...
`docs/firmware/DSP.md`; the chronological log lives in `docs/history/NOTES.md`, and the tools are
mapped in `docs/remixer/TOOLING.md`.
...
- **ELUP layer** (`.bin`): XOR obfuscation with feedback (fixed embedded constants
  ...
  Reimplemented in `tools/build/bin_decode.py`; **the firmware checksum validates** → correct.
...
1. Saves ALL of the current task's registers (D0–D7, A0–A7) into its TCB `_DAT_800068fc`
   ...
   travels in the exception frame the `rte` pops). Full layout in `docs/firmware/RTOS_FORK.md` §2.
...
retracted. ✅ 8 Sep 2026 (`docs/firmware/COLDFIRE_PORT.md` O8, the emulated join runs the
...
- **Sequencer clock**: the periodic source that dispatches the trig-processor `FUN_400977cc`
  ...
  *Partially closed*: the tempo/project-BPM path on the ColdFire side has since been read
  end-to-end for the tempo-sync work (`docs/firmware/DSP.md` §6c); the trig dispatch source itself
  remains unread.
- ~~DSP program load: where the blob that `FUN_40001d4c` uploads comes from (an OS
  ...
  module format, load map and boot sequence are fully mapped in `docs/firmware/DSP.md` §§1–3
  (`tools/build/dsp_modmap.py` recovers the map).
