...
`modules/ccpage2` adds CC 62-67 → FX2 page-2 (§ docs/firmware/MIDI.md). On hardware
...
- **Audio A/B** (`tools/hw/hw_bus_test.py`, T5 soloed, synchronous paired
...
**Tooling that made this measurable.** `tools/hw/hw_bus_test.py` (synchronous
...
`tools/hw/hw_knob_sweep.py` runs the harness over every page-1/page-2 knob.
