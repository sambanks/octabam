...
`docs/firmware/midi_re_cc.md`, `docs/firmware/midi_re_scene.md`, `docs/firmware/midi_re_note.md` (kept as
the evidence record; this file is the plan).

Confidence markers as in `docs/firmware/CHIP.md`: ✅ measured / decompiled, 🟡 inferred,
...
4. `midi` off `main` @ `1a1929b`; caves in `modules/tempo-sync/` (originally `cf/`); the level-voicing session
   stays on `main` (parked, `tools/hw/level_cap.py`).
...
### The DSP record ✅ (corrects `docs/firmware/DSP.md` §6c-i)
...
select are gone. `tools/verify/verify_midi.py` checks the note against the PTCH knob
...
1. ✅ **Tier-1 page 1 on hardware (24 Aug, no flash)**: CC 40 on T3's channel moved BusDelay TIME, CC 7 moved LEVEL; the OT echoes TIME as CC 40 with CC OUT on. Driven from `tools/hw/ot_midi.py` (CoreMIDI via ctypes, no deps) through a Midihub `FROM A → FILTER(drop realtime) → OCTATRACK` pipe. Previously: ⬜: CC 40–45 on T1's channel
