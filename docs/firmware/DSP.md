...
byte (`docs/firmware/COLDFIRE_PORT.md`, O8). Payload B's entry P:0x38000 is written by
...
`tools/build/dsp_modmap.py` tries both and keeps the one that parses. The result is not
a judgement call:
...
A wrong record layout does not land exactly on the end of the blob twice.
    ...
    python3 tools/build/dsp_modmap.py        # prints the full map
...
python3 tools/build/dsp_modmap.py                       # the full module map
...
`tools/build/dsp_disasm_all.py` disassembles every P module at its correct address
...
provenance in **`docs/firmware/EXTERNAL.md`** — this is other people's work, adopted
...
* **No delay buffer.** Every non-effect module characterised: `0x2bf`, `0x3a1`,
  ...
  🟡 **Two of those labels are corrected by external work** (`docs/firmware/EXTERNAL.md`,
...
running (`DELAYPROBE=stock|silence|send` in `tools/build/build_bus.py`). All three
...
mixdown — `docs/firmware/EXTERNAL.md`. The caution in this paragraph stands on its own
...
## 6b. Harness status (`tools/harness/dsp_host`)
...
renders the full bus — `docs/remixer/HARNESS.md`.)*
...
30 Aug 2026 — see `docs/firmware/EXTERNAL.md` §5.
...
(`docs/firmware/EXTERNAL.md`). Note the standing internal contradiction this leaves
...
**reads** — but, corrected 24 Aug 2026 (`docs/firmware/midi_re_note.md`): the frame
...
`dsp56kEmu` ships an assembler as well as an emulator. `tools/harness/dsp_host/dsp_asm.cpp`
...
* **and was validated in `tools/harness/dsp_host` first**, where X memory is real: it
...
The probe runs identically in `tools/harness/dsp_host` and on hardware using the same
...
Blackhole-class space possible. `tools/build/gen_reverb.py` now sizes against it:
...
One trap when fixing it in `tools/build/gen_reverb.py`: the offset is interpolated into
...
`tools/harness/dsp_host/dsp_host.cpp -inst N -guard` runs N instances the way the
