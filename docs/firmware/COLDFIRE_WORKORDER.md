...
> **Read `docs/firmware/COLDFIRE_PORT.md` first**: what exists, what was measured, and
> the rule that governs everything here — **route A (`tools/emu/emu_rtos.py`) is
...
1. **Write the gate before the code, and watch it fail.** O2 did this for the
   ...
   nothing (`tools/verify/verify_bus.py`'s selftest exists for exactly this reason).
...
.venv/bin/python3 tools/emu/emu_rtos.py --project out/projects/OCTABAM_ONEAUX \
...
only), `_tick_timers`, and the idle rule at `MAIN_SPIN` — `tools/emu/emu_rtos.py`
...
ticks. The oracle is `tools/emu/emu_frames.py --project out/_testproj --frames
...
**Translates:** `tools/emu/emu_card.py` (FAT16 + the ATA task-file model at
...
3. **Nine things the vendored DSP emulator does that the firmware cannot live
   with** are in `tools/patches/dsp56300.patch` (DO loops nested in one call, JIT-only
...
1. Link `dsp56kEmu` into `ot_emu`, two cores, shared-window patch (mechanical —
   `tools/harness/dsp_host` already does it).
...
4. Make the eDMA MOVE data — route A's model deliberately moves none — so the
   frame exchange carries the 336/64/32-word records `docs/firmware/DSP.md` names.
