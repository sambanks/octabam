...
`0x400d24d0..0x400d2ce0`. The generic cave installer in `tools/build/build_bus.py`
...
- **The staged page**, which is what the panel draws:
  ...
  `docs/firmware/midi_re_cc.md` already knew `0x46c7d244[2*slot+1] = 0x14` as the
...
`docs/firmware/MIDI.md` records that page 2 is unreachable from CC and cannot be
...
- ✅ **The page-2 array is `+0x8ef5a`, stride 30 per track**, which is the
  same stride the PROJECT FILE uses for page 2 (`tools/hw/ot_project.py`,
...
`docs/firmware/PARAM_PAGES.md` records from the other direction and was taken to mean
...
`docs/firmware/midi_re_cc.md` §7). The screen was self-consistent (draw and edit both
...
no TIME formatter (`build_bus.py`, ~800 B on the rig, `docs/remixer/MODULES.md`),
