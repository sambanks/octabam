...
`tools/emu/emu_rtos.py` runs the firmware's own scheduler: the handoff trap
...
- **Reading SR through the API at a burst boundary corrupts the condition
  ...
  translator's own path; `tools/emu/emu_rtos.py --selftest` pins it. Writing SR
...
One new module, `tools/emu/emu_rtos.py`, on top of `emu_bringup` (unchanged) and
...
- **M6a — first real context switch.** ✅ **DONE 6 Sep 2026** *(judgment)*
  `tools/emu/emu_rtos.py`: the handoff trap dispatched post-burst (the boot's
...
- **M6d — input.** **Done 6 Sep 2026, and the premise above was wrong** — §9
  ...
  shortcut in `MAINMENU.md`. `tools/emu/emu_rtos.py`'s `press_play_live()` /
...
.venv/bin/python3 tools/emu/emu_rtos.py --selftest      # the SR-read trap, pinned
```
...
run against it (`tools/emu/emu_frames.py --project out/_testproj --frames 400
...
The chip is the MCF5445x family (`docs/firmware/CHIP.md`: MCF54454; the register map
...
that reads it. `tools/emu/emu_rtos.py`'s `TASK_NAMES` swapped the two labels to
...
```sh
python3 tools/hw/ot_project.py machine-type <project> <bank> <part> <track> <type>
...
```sh
python3 tools/hw/ot_project.py pattern-trig <project> <bank> <pattern> <track0> <step> [mask]
python3 tools/hw/ot_project.py pattern-diff <projectA> <projectB> <bank>
```
...
.venv/bin/python3 tools/emu/emu_rtos.py --project out/_recproj --set OCTABAM --name RIG \
...
`seqprobe.py`); the tool changes are in `tools/hw/ot_project.py`.
...
factor is ignored in fractional mode on both. **`tools/patches/unicorn_emac_fractional.patch`**
...
extension word**. `tools/patches/unicorn_emac_fractional.patch` now carries both
...
3. The fractional patch's extraction (`get_macf`) shifted the accumulator
   ...
   `tools/patches/unicorn_emac_fractional.patch` (arithmetic shift, signed compare,
...
**Firmware:** the `.bin` decodes (`tools/build/bin_decode.py`, then a small
...
breaks.** `tools/emu/ot_emu` was rebuilt clean (a peer session held the shared
...
`0x46c2e9c0`/`0x40A955E0`, `docs/firmware/RTOS_FORK.md` §10.18) for tag 22's build,
...
**New tool capability.** `tools/emu/ot_emu/main.cpp` gets `--mem-dump
...
### 10.26 §10.25's ruling-out may be premature: `tools/emu/ot_emu` has no clock-lock modeling of any kind — not even the compensation §10.14 gave route A (9 Sep 2026 — read, not measured)

Checked before spending more session time on the lane-table trace §10.25
left open. `grep` across `tools/emu/ot_emu/*.cpp`/`*.h` for `arm_phase`,
...
`tools/emu/emu_rtos.py` (route A); the C++ port §10.21–§10.25 have been
running against (`tools/emu/ot_emu`) has neither the bug's compensation nor,
...
`--arm-phase-fix` (or a real MIDI-clock model) to `tools/emu/ot_emu`, not a
...
directly, not remotely through Bryan. Flash 7 (`tools/hw/hw_flash7.py`) is
...
(`tools/hw/hw_flash7.py`'s clock + source; capture analysed off-line). ⚠
...
silent (🟡 inferred, not measured):** `tools/emu/emu_rtos.py`'s `Edma` class
...
exact check under the C++ port (`tools/emu/ot_emu`), which runs a real DSP and
