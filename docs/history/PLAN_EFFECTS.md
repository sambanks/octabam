> ### This was `PLAN.md` until 10 Sep 2026 — the DSP-effects programme.
> Moved here unchanged when the repository's purpose became the remixer
> (`PLAN.md` at the root is the plan now). Everything below is still the
> record of the effects side: its open items are open, its gates apply,
> and `PLAN.md §n` / `PLAN §n` references elsewhere in the tree point at
...
> without sends. `docs/effects/BUS.md` "The one aux bus" is the record;
> `tools/verify/verify_onebus.py` (in `make check`) measures every property with
...
> core 0 1,570 instructions/sample, core 1 702 (`docs/remixer/HARNESS.md`). The
...
**This is the cold-start document — read it before `docs/effects/XBUS.md`**, which is
...
- **BusVerb** — an eight-line FDN reverb (ROOM/PLATE/BIG), shimmer, a gated
  ...
  (payload A / core 0); any track can send into it. `docs/effects/REVERB.md`.
...
- **Tempo sync (R56, 24 Aug 2026, ON THE UNIT, confirmed)** — two ColdFire *code*
  ...
  slew, snap and labels confirmed on the unit the same evening; levels may want tuning. Lessons in `docs/firmware/DSP.md` §6c
  (never init-build tables in Y through `(r1)+` — R48–R50 killed every
  voice). `docs/firmware/PARAM_PAGES.md` §7 decodes the formatter ABI.

- **The whole rig renders locally, on BOTH cores (7 Sep 2026, tier 1 of
  the emulator uplift).** `tools/harness/dsp_host` boots payload A *and* payload B
  ...
  payload-A addresses). `tools/harness/rig_render.py` / `make render-rig` drives
  ...
  (`tools/verify/verify_twocore.py`, in `make check`) — payload B's `$38000`
...
`docs/remixer/MODULES.md` "Keeping STOCK effects in the chooser". Later the same
...
(`tools/patches/dsp56300.patch`, MPYRI) and still passes at +6 dB at zero settings
...
`docs/remixer/MODULES.md` is the contributor guide.
...
- **The reverb LFO-block roll is built and PARKED, not available.**
  ...
  not more reading.
- **OMR memory map** (`docs/firmware/CHIP.md` §3): Fig 3-3 doubles P, 8K → 16K,
...
stands. `docs/firmware/CHIP.md` §2, `docs/firmware/COLDFIRE_PORT.md` O8 "the ESAI rate".
...
- **"Clipping on both effects" (24 Aug 2026) — RESOLVED UPSTREAM, measured
  ...
  (`tools/hw/level_cap.py`, `tools/hw/ot_midi.py`): the dry mix was flat-topping
...
- **R58 (tag 77, ON THE UNIT 24 Aug — hardware-ratified: lean 8.0 -> ~4.4 dB, wet +3.8 dB, no clipping): PING
  ...
  effects. `docs/effects/VOICING.md` R58. Hardware ear-ratify on next flash.
- **Full MIDI-driven hardware voice pass DONE (24 Aug 2026 evening,
  Sam's set, R57)** — see `docs/effects/VOICING.md`. No clipping at any page-1
...
**The two facts that make it possible are measured** (`tools/build/dsp_reach.py`
disassembly of payload A, against the module records and the spans in
`docs/firmware/DSP.md` §8):
...
- **Every one is SELF-CONTAINED.** No control flow leaves an effect's own
  ...
  its record into four small blocks, `docs/firmware/DSP.md` §8. Its span is the true
  one above.)
...
id and cursor tables written (`docs/remixer/MODULES.md`). It costs no words; the
...
`docs/firmware/CHIP.md` §2); the sweep is to re-measure it against whatever bank FX1
...
over SDRAM rings at `0x4F502C10` (`docs/firmware/EXTERNAL.md`, Bryan T). So the signal
...
- The memory map is already decoded (`docs/firmware/ARCHITECTURE.md` §7), the DSP
...
- The MAIN MENU table system is decoded (`docs/firmware/MAINMENU.md`), so the first
...
**Tier-0 bring-up is underway and de-risked** (`docs/remixer/EMU.md`,
`tools/emu/emu_bringup.py`, 31 Aug 2026): with SP+SR seeded and MMIO modelled,
...
JIT-cache gotcha are in `docs/remixer/EMU.md`.
...
2026 — `docs/firmware/RTOS_FORK.md`**: kernel decoded, feasibility spike passed, five
...
emulation) — `tools/emu/emu_card.py`, `make emu-card PROJECT=<dir>`, `docs/remixer/EMU.md`
...
image file (`docs/firmware/ARCHITECTURE.md` §5 has the opcodes and task-file
registers); the image is a FAT16 volume with a project from
`tools/hw/ot_project.py`. The open risk is the async completion path through
...
(`tools/emu/emu_rtos.py`, `docs/firmware/RTOS_FORK.md`) runs the firmware's own
scheduler.
...
`tools/emu/emu_rtos.py --project out/_testproj --set OCTABAM --name RIG
...
(RTOS_FORK §7, §8.3). Reproduce with `tools/emu/emu_rtos.py --project <abs dir> --set OCTABAM
--name RIG --load-project --ms 6000` (absolute path; a tilde inside the
quoted make variable is not expanded), or `tools/emu/emu_rtos.py --selftest`
...
the aliases, and `tools/build/mode_names.py` emits a MODE formatter that rewrites
...
the page draws that slot. `tools/verify/verify_modenames.py` (in `make check`) calls
...
pass that changed that (`tools/build/build_bus.py`, the section after the cave
patches).
...
**The bytes are emitted, not assembled** (`tools/build/label_fmt.py`). A CavePatch
...
**Verified without a flash.** `tools/verify/verify_labels.py` (in `make check`)
...
the string-capture hook cannot read (`docs/remixer/EMU.md`), so the emulator proves
...
frames inside the audio interrupt (`docs/firmware/EXTERNAL.md` §1), so a cave in
...
hosts going quiet only while a return is live. `docs/effects/BUS.md` "The returns";
...
- **MODE is on page-2 slot 6 in both bus engines (4 Sep 2026, unflashed;
  ...
  main-menu bus screen needs (`docs/firmware/MAINMENU.md` §9c-ii: nine rows WITH
...
- **Cross-core bus: three defects found and fixed, all hardware-confirmed**
  ...
  caveats, from `docs/effects/XBUS.md`: the fix assumes the cores are **rate-locked**
...
- ✅ **"the workbench" was a name nobody reviewed, and it is now "the
  ...
  `tools/remix/`, the `Remix` dataclass). `docs/remixer/REMIXER.md` is the manual.
...
- ✅ **The hardware-mpy caveat is CLOSED**: silicon decay-vs-TIME matches the
  emulator within 13% at three points (`docs/effects/CAPTURE.md`), so the emulator's
...
- Flash discipline: bump `BUILD` every flash, power-cycle before judging
  anything, recovery path read first — `docs/remixer/FLASHING.md`.
- Voicing is judged by ear, level-matched, A/B/A/B, wet-only, logged in
  `docs/effects/VOICING.md`. A reverb is finished when a long tail decays without a
