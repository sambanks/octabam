...
bank. Built by `tools/build/build_bus.py`; source `modules/busverb/reverb_server.asm`.
...
> (`dsp/reverb88.asm` via `tools/build/build_reverb.py`) replaced stock DARK REV in
...
|---|---|---|---|---|
| 1 | 0 | AUX | `r6+$0` | **this track's own send into the one aux bus** (7 Sep 2026; the v8 `-DEL` machinery re-slotted): dry mono × knob, 3-bit headroom, into the AUX accumulator, counted through the flag word `Y:0x941`. Default 0 (phantom-client rule). The host's dry always passes at unity underneath |
| 1 | 1 | TIME | `r6+$1` | feedback → RT60 (slot 0 until the one-aux re-slot) |
| 1 | 2 | MOD | `r6+$2` | modulation depth |
| 1 | 3 | SIZE | `r6+$3` | scales all four tap lengths, within the current MODE |
| 1 | 4 | TONE | `r6+$4` | **LO + HI on one knob** (v8, 5 Sep 2026; was HP). 0..64 is the old LP 0..127 with HP off (dark → flat); 64..127 is the old HP 0..126 with LP wide open (flat → thin). **64 = the old HP 0 / LP 127 exactly** (render bit-identical; TONE 63 ≡ old LP 126 likewise). Merged so slot 4 could carry the host's send |
| 1 | 5 | MIX | `r6+$5` | **the stage crossfade** (7 Sep 2026; IN until then): out = in × (1 − MIX) + wet × MIX, where `in` is the chain input (the delay's output while the delay is live, else the aux). 127 = wet only (the old return); 0 passes the chain input through. The host prints wet × MIX under its dry. The IN-keyed wet makeup (×(1 + 2·IN)) went with IN |
| 2 | 6 | MODE | `$c` bits 16-23 | **stepped select**: 0 ROOM, 1 PLATE, 2 BIG. Slot 6 since v7 (4 Sep 2026; slot 7 / bits 8-15 before): an EVEN slot is one the panel's page-2 knob editor can write, so a main-menu screen can set MODE through the firmware's own routine (`docs/firmware/MAINMENU.md` §9c-ii) |
| 2 | 7 | SHMR | `$c` bits 8-15 | shimmer amount, 0 = off (slot 6 / the knob field until v7). A 128-count knob in a companion field — stock does this (FILTER DIST on 11, CHORUS FBLP on 9) and DSP.md records two full-range companion knobs measured on the unit; the 10 Aug "near-boolean" reading predates the field-map and formatter fixes. ✅ Tag 84 (4 Sep 2026): smooth on the unit |
...
`tools/verify/verify_menu.py` now enforces this.
...
A/B'd through `tools/harness/render_reverb.py` on a real pitched synth loop — same
...
```sh
python3 tools/build/build_bus.py                          # -> out/mainos_bus.bin
```
...
```sh
python3 tools/build/build_reverb.py dsp/reverb88.asm      # RETIRED path
...
python3 tools/build/make_bin.py out/elek_v88.bin -o out/OCTATRACK_V88.bin
```
...
state words, dirty memory, and a nonzero split. `tools/build/dsp_modmap.py`'s
...
`tools/harness/dsp_host` runs the real assembled instruction stream with exact
...
time**. `tools/harness/render_reverb.py` wraps it:
...
Counted by `tools/build/cycle_count.py`, which assembles each server, injects a label
...
**Do not measure this in `tools/harness/dsp_host`.** Its `instructions/sample` figure
...
and `docs/effects/VOICING.md` records why each moved. The table is kept below because
...
3. ~~Measure real cycle cost with both effects live~~ — **counted**, and the
   ...
   earlier records carried. `tools/build/cycle_count.py` makes it reproducible.
