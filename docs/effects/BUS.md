...
(Cross-bus sends section below). `tools/build/build_menu.py` replaces
...
`BusVerb`, `BusDelay`, `Send`. Build with `python3 tools/build/build_bus.py`
(writes `out/mainos_bus.bin`), check with `python3 tools/verify/verify_menu.py`, then
wrap and flash per `README.md` §3. `BUILD_TAG` in `tools/build/build_bus.py` is the
...
| | |
|---|---|
| build | `tools/build/build_bus.py` — DSP placement + all five ColdFire tables, both payloads |
| verify | `tools/verify/verify_menu.py` — menu tables vs the real decompiled chooser |
...
`tools/build/build_bus.py`) — bump it every time a `.bin` is wrapped; otherwise
there is no way to tell which firmware is on the unit.

Cycle budget is **measured, not guessed** — by `python3 tools/build/cycle_count.py`,
which is the only correct way to get it: `tools/harness/dsp_host` CANNOT measure this
...
parity that leaves. `tools/harness/dsp_host` gained a `-peeky <addr,...>` flag to
...
as its offset and may not. Verified in `tools/harness/dsp_host` — 1 block, `-split
...
flashed).** `tools/harness/dsp_host` gained `-pokey addr=val,...` (seed Y memory
...
(`tools/harness/dsp_host`'s `-in` duplicates one stream to L and R, and the
...
**Built and verified, task 11 (`tools/build/build_menu.py`, `tools/verify/verify_menu.py`).
...
(`tools/build/build_reverb.py`) reuses DARK REV's *own* id and descriptor with no
...
fresh address (`tools/build/build_menu.py`'s `CLONE_BASE`, a confirmed-free cave),
...
`tools/verify/verify_menu.py` diffs SPRING/DARK/FILTER's own descriptor bytes
...
Re-verified in `tools/harness/dsp_host` at the new offset: DRY=110 lands the same
...
**Built and verified, task 13 (`tools/build/build_bus.py`).** The three servers'
...
in *Start here*, and `tools/build/build_bus.py`'s docstring for the layout.

| entry | id | P-code donor | budget |
|---|---|---|---|
| `SEND` | `0x03` | ~~CHORUS~~ (329 words, confirmed no inbound branches — `tools/build/build_dspprobe.py`'s relocatability scan) | 127/329 |
| `DELAY SERVER` | `0x01` | PLATE REV (594 words, confirmed no inbound branches — `tools/build/build_reverb.py`'s own note) | 453/594 |
| `REVERB SERVER` | `0x02` | SPRING + DARK's front (2037/2130 words combined, the same budget `tools/build/build_reverb.py` already proved safe on hardware) | 1215/2037 |
...
**One trap the repack exposed:** `tools/harness/render_reverb.py` had the engine's
...
avoids that: this is the *exact* null stub `tools/build/build_dspprobe.py` already
...
closing the last open item from task 9/10: `tools/build/build_bus.py` substitutes
`$30000`→`$38000` in the source text before assembling payload B's copy (one
occurrence, checked). Verified statically (not via `tools/harness/dsp_host`, see
...
**Verified in `tools/harness/dsp_host`, payload A**: all three ids, called at their
...
B could only be checked statically**: `tools/harness/dsp_host`'s boot-setup step
...
which `tools/verify/verify_menu.py` replicates directly against the built image and
...
`tools/harness/dsp_host` only exercises the DSP chip directly (`-params` pokes `r6`
...
**Fixed** in `tools/build/build_menu.py`/`tools/build/build_bus.py`: the list is now
...
instead of garbage. `tools/verify/verify_menu.py` now checks this directly.
...
`tools/build/build_dspprobe.py` already proved runs custom DSP code correctly on a
...
Both fixes are in `tools/build/build_menu.py` and `tools/build/build_bus.py`; the DSP
code itself (`dsp/reverb_server.asm`/`delay_server.asm`/`send_client.asm`)
is untouched — re-verified in `tools/harness/dsp_host` at the new dispatch addresses
...
`tools/verify/verify_menu.py` now asserts the enabled set matches the intended
...
`0x06` is the id `tools/build/build_dspprobe.py` hardware-proved (`DSP.md` §7d)
...
mask distinctly — not by emulator test: `tools/harness/dsp_host` cannot switch an
...
garbage in an X state slot (`tools/harness/dsp_host` has `-pokey` but no `-pokex`),
...
Dumping the real table from both stock payloads (`tools/build/dsp_modmap.py
...
run in `tools/harness/dsp_host` against the real, unmodified stock table:
...
11), and `$b` is simply the next offset `tools/harness/dsp_host`'s `-params` flag
...
matches the hatch to the bit — `tools/verify/verify_twocore.py`), but the TIMING of
...
- ~~`DELAY SERVER`'s payload-B base literal is not yet parameterized~~
  **fixed, task 13** — `tools/build/build_bus.py` substitutes `$30000`→`$38000` in
  the source text before assembling payload B's copy. Checked statically
  (word-for-word against a standalone reassembly); `tools/harness/dsp_host` itself
  can't run payload B at all yet (next bullet), so this couldn't be
  re-verified the same way task 10's other offsets were.
- ~~**`tools/harness/dsp_host` cannot run payload B.**~~ ✅ **FIXED 7 Sep 2026.**
  ...
  delay renders bit-identical to the DEV copy (`tools/verify/verify_twocore.py`),
...
- ~~`tools/harness/dsp_host` cannot run payload B at all~~ — fixed 7 Sep 2026
...
1. Verify the hardcoded-base approach in isolation first — confirm in
   `tools/harness/dsp_host` (guard + dirty memory) that a server using a fixed
...
6. ~~ColdFire menu integration~~ **done, verified against the real
   decompiled chooser functions, not flashed** — `tools/build/build_menu.py`
   ...
   section above). `tools/verify/verify_menu.py` confirms the five tables agree
...
7. ~~DSP code placement + dispatch~~ **done, emulator-verified (payload A;
   payload B checked statically), not flashed** — `tools/build/build_bus.py`
   ...
   are repointed to the same null stub `tools/build/build_dspprobe.py` already
