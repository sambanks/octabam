...
distinguishable*. `docs/remixer/FLASHING.md` is the procedure; this is the schedule.
...
```bash
python3 tools/hw/ot_project.py testproj \
...
- The cursor opens **on its row**, not row 0. ⚠️ That is `FX1_ID2POS`, which
  `tools/build/build_fx1.py`'s original experiment never wrote — the one thing in
...
```bash
python3 tools/hw/ot_project.py rigproj ~/octa/backups/<trusted project> /Volumes/<card>/OCTABAM_RIG bamsep26
```
...
```bash
python3 tools/hw/ot_project.py stamp-defaults /Volumes/<card>/<set copy> bamsep26
```
...
on track 8, the stations without sends. `docs/effects/BUS.md` "The one aux bus".
...
```bash
python3 tools/hw/ot_project.py rigproj ~/octa/backups/OCTABAM_RIG_20260906_cleared /Volumes/<card>/OCTABAM_ONEAUX bamsep27
# or, for a real set on a copy:
python3 tools/hw/ot_project.py stamp-defaults /Volumes/<card>/<set copy> bamsep27
```
...
(`docs/firmware/COLDFIRE_PORT.md` O11/O12), none by guessing:
...
### The automated run (`tools/hw/hw_flash7.py`, 9 Sep 2026)
...
(`tools/hw/hw_flash7_liveclaim.py`) rather than by program change. `COLDFIRE_PORT.md`
...
`modules/cfprobe/README.md` is the full procedure; `tools/verify/verify_cfprobe.py`
...
(`docs/remixer/MODULES.md`: a select value outside its count is used as an index and
...
tools/hw/ot_project.py stamp-defaults <project> bus`, or re-select the effect on
every track of every part that carries it.**

Both bus engines moved MODE to page-2 slot 6 and SHMR / MDEP to slot 7
(`docs/firmware/MAINMENU.md` §9c-ii, `docs/firmware/PARAM_PAGES.md`). Locally bit-identical in
...
python3 tools/hw/ot_project.py stamp-slot "<project>" busverb  TONE
python3 tools/hw/ot_project.py stamp-slot "<project>" busverb  -DEL
python3 tools/hw/ot_project.py stamp-slot "<project>" busdelay -DEL
```
...
- ❌ **CONTROL showed no REVERB / DELAY rows.** The image carries the patch
  ...
  `docs/remixer/FAILURE_MODES.md`. Not diagnosed; the emulator's own menu is the
...
From `docs/remixer/FLASHING.md` and the card workflow this project already uses:
...
- [ ] Stable power.
- [ ] **A capture rig ready before the flash, not after** — `tools/hw/rec.swift`
