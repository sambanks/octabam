...
(`tools/harness/render_reverb.py --mode all`, ~8 s for all four) so nothing here costs a
...
Sam's own set on the unit (R57), driven end-to-end over MIDI (`tools/hw/hw_sweep.py`:
...
Built and gated on both cores (`docs/effects/BUS.md` "The one aux bus"), not yet
