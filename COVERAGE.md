...
| **Effects — Appendix B (17 FX)** | 🟡 | All on the DSP. The **dispatcher ABI, per-instance memory allocator and parameter path are fully reversed**, and a custom reverb replaces DARK REV and runs on all 8 tracks (`dsp/reverb71.asm`, see `REVERB.md`). The stock algorithms themselves are still undecompiled |
