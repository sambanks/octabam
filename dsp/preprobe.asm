...
; ---- MIX -- PROBE: driven by slot $c, the suspected PRE ------------------
; DIAGNOSTIC BUILD. The wet gain is taken from parameter slot $c instead of
; MIX ($5), so the slot we believe is PRE controls how LOUD the reverb is --
; a readout nobody can fail to hear, unlike a 46 ms pre-delay.
;
; Sweep every knob on both pages. Whichever one changes the reverb VOLUME
; is slot $c:
;   * the PRE knob changes it -> the slot mapping is right, the pre-delay
;     has been working all along, and it was simply too subtle to hear.
;     Ship v59 unchanged and make the pre-delay longer if more is wanted.
;   * a DIFFERENT knob changes it -> that knob is $c, and the real PRE knob
;     is elsewhere; read the slot straight off which knob moved.
;   * NO knob changes it (volume just sits at one level) -> $c is not a
;     knob at all on hardware, and the page-2 mapping needs a fresh probe.
; MIX itself stops working in this build, by design.
        move    x:(r6+$c),a
        and     #>$7f0000,a             ; knob field; AND cleans A1 only
        move    a1,x0                   ; so A2-clean before storing, or the
        move    x0,x:(r7+$20)           ; limiter mangles the gain (the v55 rule)
