...
;
; v51 = v50 with ONE change: the control call BURNS ~60 instructions before
; returning instead of rts'ing in three. The pattern across every build of
; the investigation: everything that idles ~60 instructions on the a=0 call
; (stock DARK reads its parameter slots there; every surviving stageprobe
; runs its scaffold on both calls) survives two tracks. Everything that
; returns instantly -- v44, v46, v50 -- freezes. The scaffold carrying THIS
; VERY ENGINE (stageprobe7/8) survives two tracks, so the engine is not the
; difference; the control call is the last structural delta standing.
        tst     a
        bne     audio
        do      #60,>ctlend
        nop
