...
; ---- rebuild the four delay pointers from the saved phase ----------------
    ...
    ; Setup only -- this runs once per block, not per sample.
            ...
            move    a,x:(r7+$62)            ; v75: keep f -- the in-loop
                                            ; allpasses and the modulation
                                            ; depth must both track SIZE, or
                                            ; they are wrong at small sizes
...
lf3e:
        ...
        mpy     x0,y1,a
        move    a1,x1                   ; NOT scaled by SIZE -- measured: making
                                        ; the sweep proportional HALVED flatness
                                        ; at small SIZE. Modulation smears modes,
                                        ; and small SIZE is where they are
                                        ; sparsest, so it needs MORE not less.
        asl     #$8,a,a
        move    a2,x0
...
; v75: these SCALE WITH SIZE. Fixed taps were 206% of the line they feed at
; SIZE=0 and 26% at SIZE=127 -- so the loop time stopped tracking the knob,
; and the allpass was longer than the delay behind it. Dattorro's ratio is
; ~15%; scaling by f holds it constant across the range.
        move    x:(r7+$62),x1           ; f
        move    #>$0c8800,x0            ; 401 as a fraction of 4096
        mpy     x0,x1,a
        asr     #$b,a,a
        move    #>$400,b
        sub     a,b
        move    b,x:(r7+$60)            ; 1024 - 401*f
        move    #>$12c800,x0            ; 601 as a fraction of 4096
        mpy     x0,x1,a
        asr     #$b,a,a
        move    #>$400,b
        sub     a,b
        move    b,x:(r7+$61)            ; 1024 - 601*f
