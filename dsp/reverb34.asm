; ---------------------------------------------------------------------------
; reverb v34 (runtime stage bisect on HP) -- generated by tools/gen_reverb.py, do not hand-edit
;
...
; Read X:$213 here, in process. No stash, and no absolute Y anywhere in
; the effect.
;
...
; ---- BISECT LEVEL from the spare p3 knob --------------------------------
; Two instances hang, and cycles, bandwidth, buffer region and base delivery are
; all cleared. So bisect the engine itself, at runtime, in one build:
;
;     HP <  43   tank only          (no pre-delay, no diffuser)
;     HP <  86   + pre-delay
;     HP >= 86   + diffuser         (the full engine)
;
; Set the SAME level on both tracks and raise it until they crash.
        move    x:(r6+$3),a
        asr     #$10,a,a
        move    a,x:(r7+$50)
