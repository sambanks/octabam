...
; a SECOND, much smaller offset from the same phase, for allpass 2. The tank
; offsets reach 126 samples, which would swamp a 331-sample diffuser; this
; one spans 0..15. Same phase, so it costs only the extract.
        move    x1,a                    ; the scaled triangle again
        asl     #$5,a,a
        move    a2,x0
        move    x0,x:(r7+$5a)           ; 0..15 samples
        move    x1,a
        move    #>$07ffff,x0
        and     x0,a
        asl     #$5,a,a
        move    a,x0
        move    x0,x:(r7+$5b)           ; its fraction
...
; a SECOND, much smaller offset from the same phase, for allpass 3. The tank
; offsets reach 126 samples, which would swamp a 331-sample diffuser; this
; one spans 0..15. Same phase, so it costs only the extract.
        move    x1,a                    ; the scaled triangle again
        asl     #$5,a,a
        move    a2,x0
        move    x0,x:(r7+$5c)           ; 0..15 samples
        move    x1,a
        move    #>$07ffff,x0
        and     x0,a
        asl     #$5,a,a
        move    a,x0
        move    x0,x:(r7+$5d)           ; its fraction
...
; ---- modulated allpass read offsets (v73) ------------------------------
; n5 for allpasses 2 and 3 is no longer a constant: it carries an LFO
; offset so the diffuser itself stops ringing at its own tap period. We
; MEASURED the residual resonance at lag 331 -- exactly allpass 3's tap --
; once the tank was modulated, so the diffusers became the ringing source.
        move    x:(r7+$5a),a
        move    #>537,x0
        add     x0,a
        move    a,x:(r7+$5e)            ; allpass 2: (1024-487) + offset
        move    x:(r7+$5c),a
        move    #>693,x0
        add     x0,a
        move    a,x:(r7+$5f)            ; allpass 3: (1024-331) + offset
...
; does the wrap and the base add for free: y:(r5+n5) is the tap read and
; y:(r5) is the write, so the second address build disappears entirely.
        move    x:(r7+$5e),a            ; LFO-modulated tap
        move    a,n5
        move    x:(r7+$39),a            ; phase   (also spaces the n5 write)
        ...
        move    a,r5                    ; = write address
        move    x:(r7+$1b),x1           ; input, and spaces the r5 write
        move    y:(r5+n5),b             ; d0, at (phase - tap - offset) mod 1024
        move    x:(r7+$60),a            ; d1 = last sample's d0 -- the read
        move    b,x:(r7+$60)            ; pointer advances 1/sample here too,
        move    x:(r7+$5b),y1            ; so the same carry identity holds
        sub     b,a
        move    a,x0
        mpy     x0,y1,a
        add     b,a                     ; interpolated d, no zipper on the
        move    a,b                     ; block-rate offset change
        move    b,x0
        mpy     x0,y0,a
        move    x1,x0
        add     x0,a                    ; v = x + g*d
...
; does the wrap and the base add for free: y:(r5+n5) is the tap read and
; y:(r5) is the write, so the second address build disappears entirely.
        move    x:(r7+$5f),a            ; LFO-modulated tap
        move    a,n5
        move    x:(r7+$39),a            ; phase   (also spaces the n5 write)
        ...
        move    x:(r7+$1b),x1           ; input, and spaces the r5 write
        move    y:(r5+n5),b             ; d0, at (phase - tap - offset) mod 1024
        move    x:(r7+$61),a            ; d1 = last sample's d0 -- the read
        move    b,x:(r7+$61)            ; pointer advances 1/sample here too,
        move    x:(r7+$5d),y1            ; so the same carry identity holds
        sub     b,a
        move    a,x0
        mpy     x0,y1,a
        add     b,a                     ; interpolated d, no zipper on the
