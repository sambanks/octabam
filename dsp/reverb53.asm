...
; PRE-DELAY BYPASSED. v46 works on one track and freezes on two, and this was
; the only indexed modulo access left in it -- y:(r5+n5) under m5, plus a
; post-increment write. dsp/stageprobe.asm does the same volume of delay-line
; traffic on two tracks with every M register linear and never freezes.
;
; Nothing replaces it: r7+$1b already holds the mono input, which is exactly what
; the diffuser wants, so removing these seven instructions leaves the signal path
; intact with zero pre-delay. Everything else in this file is byte-identical to
; the build that works on track 1, so a two-track result here is about the modulo
...
; v53 = v50 with TWO INSTRUCTIONS added, nothing else: the phase is saved
; TAGGED -- $2c0000 | phase -- and the load path already masks $7ff, so the
; reverb's behaviour is bit-identical. The hypothesis, from the whole build
; history: $83 is a host-owned field that stock effect layouts avoid, which
; is why it "persists". Every surviving build leaves huge tagged values
; there; every freezing build (v44/v46/v50/v51/v52) leaves the bare phase,
; 0..0x7ff, forever. If the host reads that field -- plausibly only when
; two FX2 instances are active -- a small value may look meaningful (an
; index, a pointer) where a tagged one falls harmlessly out of range.
        move    r1,a
        move    #>$7ff,x0
        and     x0,a
        move    #>$2c0000,x0
        add     x0,a                    ; tag | phase; the load masks it off
        move    a,x:(r7+$83)
dry:
        move    #>$ffffff,m0
        move    #>$ffffff,m1
