...
;
; v81: MOD now has a FLOOR and cannot reach zero. With no modulation the
; tank is completely static, and at small SIZE its modes sit ~24 Hz apart
; with nothing to smear them -- which rings like a spring. ~5 samples of
; residual sweep breaks that up without being audible as an effect. Every
; commercial reverb does this; a truly static tank is a degenerate case,
; not a useful setting.
        move    x:(r6+$1),x0
        move    #>$6b0000,y1            ; range 0.836
        mpy     x0,y1,a
        move    #>$140000,x0            ; floor 0.156 -- ~5 samples of sweep
        add     x0,a
        move    a,x:(r7+$28)
...
; 8x what it would be per sample, because the LFO is stepped once per block.
        ...
        asr     #$b,a,a                 ; the RATE knob ($d) -- if it is live at
                                        ...
                                        ; v79: DECOUPLED. MOD used to drive the
                                        ; rate as well as the depth, which was a
                                        ; workaround from when the depth was 7
                                        ; samples and inaudible. It forced deep
                                        ; and fast to rise together -- the
                                        ; seasick combination -- and made a slow
                                        ; deep swell or a fast shallow shimmer
                                        ; unreachable. MOD is depth, RATE is
                                        ; rate, as everywhere else.
        move    #>$400,x0               ; floor ~0.34 Hz, so the LFO always moves
        add     x0,a                    ; even at RATE=0; full RATE is ~1.7 Hz,
                                        ; around Dattorro's ~1 Hz.
        move    a,x:(r7+$2f)
...
lf3e:
        ...
        asl     #$7,a,a
        move    a2,x0
        move    x0,x:(r7+$21)           ; integer offset, 0..31 samples
        move    x1,a
        move    #>$01ffff,x0            ; fraction below the integer step
        and     x0,a                    ; asl #6, NOT #7 -- see the header
        asl     #$6,a,a
        move    a,x0
...
lf4f:
        ...
        asl     #$7,a,a
        move    a2,x0
        move    x0,x:(r7+$23)           ; integer offset, 0..31 samples
        move    x1,a
        move    #>$01ffff,x0            ; fraction below the integer step
        and     x0,a                    ; asl #6, NOT #7 -- see the header
        asl     #$6,a,a
...
lf50:
        ...
        asl     #$7,a,a
        move    a2,x0
        move    x0,x:(r7+$56)           ; integer offset, 0..31 samples
        move    x1,a
        move    #>$01ffff,x0            ; fraction below the integer step
        and     x0,a                    ; asl #6, NOT #7 -- see the header
        asl     #$6,a,a
...
lf51:
        ...
        asl     #$7,a,a
        move    a2,x0
        move    x0,x:(r7+$58)           ; integer offset, 0..31 samples
        move    x1,a
        move    #>$01ffff,x0            ; fraction below the integer step
        and     x0,a                    ; asl #6, NOT #7 -- see the header
        asl     #$6,a,a
...
; occupy. They share m5 = $3ff with the input diffusers, so no M write.
        ...
        move    #>623,a
        move    a,x:(r7+$60)            ; 1024 - 401
        move    #>423,a
        move    a,x:(r7+$61)            ; 1024 - 601
