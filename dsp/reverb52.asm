...
;
; v52 = v50 with ONE change: the control call DOES WHAT STOCK DARK DOES --
; it reads the parameter slots and latches them into r7 state. The evidence
; trail: every build that writes r7 slots on the control call (stock DARK,
; every surviving stageprobe) runs two tracks; every build whose control
; call is an instant rts (v44/v46/v50) freezes; and v51 proved a pure
; 60-instruction nop burn there does NOT protect -- so it is the TRAFFIC,
; not the time. The stores land in $20..$27, which the audio call freely
; overwrites with its own derivations, so the reverb is unchanged.
        tst     a
        bne     audio
        move    x:(r6),x0
        move    x0,x:(r7+$20)
