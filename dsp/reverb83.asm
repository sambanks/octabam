...
; is labelled LP and now carries the high cut, which is what it says.
        move    x:(r6+$1),x0
        move    #>$6b0000,y1            ; v83: MOD has a FLOOR. With no
        mpy     x0,y1,a                 ; modulation the tank is static and its
        move    #>$140000,x0            ; modes ring; ~2 samples of residual
        add     x0,a                    ; sweep breaks that up inaudibly.
        move    a,x:(r7+$28)            ; range is now ~2..15 samples
...
; ---- RATE, from r6+$d, MASKED so it is safe either way ------------------
; Stock never reads $d, so we cannot be sure it is a knob. But masking the
; knob field first -- exactly what stock does with PRE at $c -- makes the
; value safe whatever it holds: the result is always 0..127, so the worst
; case is a rate that does not respond, never a rate that runs away.
;
; 0.25 Hz at the bottom so the LFO always moves, ~1.2 Hz at the top, which
; brackets the classic reverb range. The four lines then run at their own
; multipliers of that.
        move    x:(r6+$d),a
        and     #>$7f0000,a             ; knob field only -- garbage-proof
        move    a1,x0                   ; A2-clean before it feeds arithmetic
        move    x0,a
        asr     #$c,a,a                 ; 0..127 -> 0..2032
        move    #>$300,x0               ; floor ~0.25 Hz
        add     x0,a
        move    a,x:(r7+$2f)
...
lf3e:
        ...
        move    x1,a
        move    #>$03ffff,x0            ; fraction below the integer step;
        and     x0,a                    ; the shift is ALWAYS one less than the
        asl     #$5,a,a                 ; integer's, or it overflows the sign bit
        move    a,x0
