; ---------------------------------------------------------------------------
; MEMORY TRAFFIC PROBE (v31 + X-access burn) -- generated by tools/gen_reverb.py, do not hand-edit
;
...
; ---- MEMORY TRAFFIC PROBE ----------------------------------------------
; The nop burn showed one instance tolerates ~508 extra instructions a sample,
; so instruction throughput is not the limit. But two instances hang even when
; forced to share a single buffer region, which rules out memory extent and
; per-instance addressing too.
;
; What the nop burn did NOT test is memory traffic. This reverb makes ~150
; x:(r7+$xx) scratch accesses per sample, because nearly every intermediate goes
; through memory; DARK REV's short block loops keep intermediates in registers.
; Two instances would be ~300 accesses a sample.
;
; Same knob (p3 / HP), same count, but the loop body now does two X-memory
; accesses per iteration instead of two nops. If this dies at a much lower knob
; setting than the nop version did, the constraint is bandwidth, not cycles --
; and the fix is to stop routing every intermediate through r7.
; KNOB IS INVERTED: burn = (127 - HP) * 16. The previous probe was swept to 127
; and the setting persists, so a non-inverted knob boots straight into maximum
; burn and hangs before it can be turned down. This way HP=127 is zero burn, and
; sweeping DOWN adds load.
        move    #>$7f,a
        move    x:(r6+$3),b
        asr     #$10,b,b
        move    b,x0
