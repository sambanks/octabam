...
; ---- rebuild the four delay pointers from the saved phase ----------------
    ...
    ; Each tap is held as a fraction of the line and scaled by f, then shifted
    ; back to an integer: tap = 3134*f on the longest line.
    ;
    ; RESCALED in v62, because the old constants ran the tap off the end of the
    ; line. `mpy` is a FRACTIONAL multiply -- it doubles -- and the old f spanned
    ; 0.125..1.861, so the nominal 1567 landed at knob ~27 and everything above
    ; that exceeded the 2048-word line. The `and #$7ff` then wrapped it, and SIZE
    ; went NON-MONOTONIC: turning it up far enough made the space smaller again.
    ;
    ; f now spans 0.08..0.653, chosen so the longest tap tops out at 2046 -- one
    ; short of the modulo limit, and one short of what |n1| may be. That gives
    ; 251..2046 samples, 5.7..46 ms on line 0, and every tap stays proportional.
    ; Setup only -- this runs once per block, not per sample.
            move    x:(r6+$2),x0
            move    #>$24f000,y1            ; doubled by mpy -> 0.5776 * size
            mpy     x0,y1,a
            move    #>$0a3d00,x0            ; + 0.08 floor
            add     x0,a
            move    a,x1                    ; f = 0.08 .. 0.653
...
; -- allpass 0: base+0x2000, tap 907 (20.6 ms) --
        ...
        add     x0,a
        move    a,r5
        move    #>$200000,y0            ; coefficient. $200000 doubles to a true
                                        ; 0.5 -- `mpy` is a FRACTIONAL multiply.
                                        ; $400000 here made it 1.0, which is a
                                        ; degenerate allpass: a comb, not a
                                        ; diffuser, and part of why the tank
                                        ; sounded metallic.
        move    y:(r5),b                ; d
        move    b,x0
        mpy     x0,y0,a
...
; ---- four taps, damped inside the feedback path -------------------------
        ...
        move    x:(r7+$3a),b            ; line 0 read, tap from SIZE via n1
        move    x:(r7+$1f),y0
        move    y:(r1+n1),a             ; modulo read -- one word, and SIZE
                                        ; finally reaches this line: n1 was
                                        ; computed every block and thrown away
        sub     b,a
        move    a,x0
        mpy     x0,y0,a
        add     b,a
...
; -- line 2 modulated: base+0x1000, tap 977, interpolated --
    ...
    ; holding HI for the lines below.
        ...
        move    x:(r7+$3d),b            ; line 3 read, tap from SIZE via n4
        move    x:(r7+$1f),y0
        move    y:(r4+n4),a             ; modulo read, same as line 0
        sub     b,a
        move    a,x0
        mpy     x0,y0,a
        add     b,a
