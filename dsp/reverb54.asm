...
; v54 = v50 with ONE change: the control call restores m0..m5 to linear
; before returning. The last untested single delta between the surviving
; scaffolds (which run their M epilogue on EVERY exit) and the freezing
; builds (which leave M untouched on a=0). v51 killed time, v52 killed
; r7-write traffic, v53 killed the $83 value, and the disassembly killed
; every host-reads-$83 theory -- the host never references $83 at all;
; it is stock DARK's own warm-up counter, and stock runs its body on BOTH
; calls. M-on-control is what remains.
        tst     a
        bne     audio
        move    #>$ffffff,m0
        move    #>$ffffff,m1
