; ---------------------------------------------------------------------------
; CYCLE CEILING PROBE -- dsp/reverb_server.asm + a knob-swept cycle burn.
; Build with BURN=1; the effect names itself BURN PROBE so it cannot be
; confused with a shipping build.
;
; WHY. Every strategic question left on this project is priced in cycles, and
; the budget we price against -- 1080/sample/DSP -- was never MEASURED. It is
; a figure stageprobe5 SURVIVED (REVERB_LOG.md: four instances, ~1080 instr/
; sample, no freeze, "well past cycleburn's 'ceiling', which was therefore
; also conservative" -- cycleburn measured ~806, for ONE instance). So "123
; free" is a floor bounded by an unknown. Pushing the other way, the bank
; model counts four FX2 slots and ignores that stock FX1 runs every frame too.
; Two live errors with opposite signs, and one flash settles both.
;
...
; MIPS per core, so "the two DSPs" are two cores of one part and each has
; ~4535 cycles/sample at 44.1 kHz. The 1080 we budget against is only ~24% of
; that, which means this probe does NOT measure the silicon: it measures where
; STOCK's leftovers run out, and that number could be far above 1080. At 8
; cycles a step the sweep topped out at 758+1016 = 1774 and could plausibly
; have run out of knob before running out of headroom -- returning "it never
; broke", which costs the whole flash. 16 a step reaches 758+2032 = 2790 and
; brackets any plausible answer. The price is 16-cycle resolution instead of
; 8, which is far below what any decision here turns on.
;
; SCALED BY n7, DELIBERATELY. The naive form (a fixed burn per proc call) does
; not measure cycles per SAMPLE: a nonzero split makes the dispatcher call
; proc TWICE a block, so the per-sample burn would silently double. And the
; split is not a transient -- every trig sets it to its landing offset and it
; PERSISTS until the next trig (this file's own header, v57), so under a
; playing pattern the doubled case IS the normal case. Iterating n7 times per
; call makes the per-sample figure exact and split-independent, which is the
; whole point of a measurement.
;
; p3'S AUDIO EFFECT IS NEUTRALISED, not merely repurposed. The LO coefficient
; is forced to 0 below, which this engine documents as an exact bypass. If the
; knob changed the sound as it climbed, "did it break" would be a judgement
; about timbre instead of a judgement about breakage.
;
...
;
; Ceiling_A gives the real per-DSP budget. Ceiling_A - Ceiling_B is what the
; rest of the machine is eating, which is the number the bank model omits.
; Turn p3 back to 0 between passes; the burn is stateless, so it resets.
;
; Note BOTH thresholds: the first value where ANYTHING changes (a tick, a
; dropout) and the value where it fully dies. They may differ, and the gap is
; itself informative -- graceful degradation and a hard freeze are different
; failure modes and would justify different amounts of nerve in spending the
; headroom.
;
; Everything below this header is dsp/reverb_server.asm verbatim except the
; two marked BURN PROBE blocks. Guard: at p3 = 0 the burn is skipped entirely
; and the render is bit-identical to reverb_server -- asserted by
; tools/verify_burn.py, which is what makes "the probe is inert when off" a
...
; ---- BURN PROBE 2 of 2: neutralise p3's audio effect --------------------
; p3 is the burn knob now, so its filter must stop moving -- otherwise every
; step of the sweep changes the timbre and "did it break" becomes a judgement
; about sound rather than about breakage. The comment directly above is what
; makes this exact rather than approximate: HP=0 gives coefficient 0, "the
; state never moves, nothing is subtracted, and the filter is bypassed
; exactly." So forcing the coefficient to 0 is the documented bypass, not an
; attenuation, and the probe's audio path is the shipping engine with LO off.
        clr     a
        move    a,x:(r7+$40)            ; LO coefficient forced to 0 (bypass)
