...
; ---- prime table B, lines 0-3 only -- lines 4-7 stay at warmup zero -----
; Without input injection the reverb dies; the Hadamard cross-group terms
; distribute energy to lines 4-7 even with zero weights here.
        move    x:(r7+$0d),a
        move    a,r1
        move    #>$7fffff,x0            ; 1.0
        move    #>$1,y0                 ; has_ap = 1
        move    x0,y:(r1)+              ; line 0 weight
        move    y0,y:(r1)+              ; line 0 has_ap
        move    x0,y:(r1)+              ; line 1 weight
        move    y0,y:(r1)+              ; line 1 has_ap
        move    #>$400000,x0            ; 0.5
        move    #0,y0                   ; has_ap = 0
        move    x0,y:(r1)+              ; line 2 weight
        move    y0,y:(r1)+              ; line 2 has_ap
        move    x0,y:(r1)+              ; line 3 weight
        move    y0,y:(r1)+              ; line 3 has_ap
; lines 4-7: warmup zero is correct (no allpass, weight zero)
; the 2 words/line × 4 lines = 8 words of zero fill were done by warmup
...
; Raising the floor costs the smallest spaces, which were the bad ones.
            ...
            ; lines 4-7 duplicate the same four tap fractions (lines 0-3)
            ; for now -- the re-voicing pass (PLAN step 1.4) will assign
            ; independent taps tuned for eight-line modal density.
            move    x:(r7+$74),x0           ; this MODE's line 0 fraction, reused
            mpy     x0,x1,a
            asr     #$b,a,a                 ; back to an integer tap (2048-word lines)
            move    #>$1,x0                 ; force the tap ODD
            or      x0,a
            move    #>$0800,b
            sub     a,b                     ; 2048 - tap
            move    b,x:(r7+$08)            ; line 4
            move    x:(r7+$75),x0           ; this MODE's line 1 fraction, reused
            mpy     x0,x1,a
            asr     #$b,a,a
            move    #>$1,x0
            or      x0,a
            ...
            move    x:(r7+$76),x0           ; this MODE's line 2 fraction, reused
            mpy     x0,x1,a
            asr     #$b,a,a
            move    #>$1,x0
            or      x0,a
            ...
            move    x:(r7+$77),x0           ; this MODE's line 3 fraction, reused
            mpy     x0,x1,a
            asr     #$b,a,a
            move    #>$0800,b
            sub     a,b                     ; 2048 - tap
...
; ---- wet added to dry, eight lines summed per channel --------------------
        move    x:(r7+$20),y1           ; wet gain
; Eight lines summed with orthogonal Hadamard-row sign patterns:
;   L = (l0-l1+l2-l3) + (l4-l5+l6-l7) + ER_L
;   R = (l0+l1-l2-l3) + (l4+l5-l6-l7) + ER_R
; Each 4-line group uses the same pattern as the old 4-line engine, and the
; two groups are independent — same dot-product-zero guarantee as before.
