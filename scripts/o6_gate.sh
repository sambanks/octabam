...
#      identical media (tools/emu/ot_emu/stage_card.py);
#   2. run route A -- the ORACLE -- and write its M6c facts;
#   3. run the port the same way and write its own;
#   4. diff them field by field, strictly (tools/emu/ot_emu/oracle.py).
#
...
"$PY" tools/emu/ot_emu/stage_card.py "$PROJECT" "$SET" "$NAME" \
...
if [ "${SKIP_ORACLE:-0}" = 1 ] && [ -f out/oracle/m6c.json ]; then
    ...
    "$PY" tools/emu/emu_rtos.py --project "$PROJECT" --set "$SET" --name "$NAME" \
