...
"""Ask the firmware what it prints for every value of every stock select.
    ...
    .venv/bin/python3 tools/build/stock_labels.py          # -> tools/remix/stock_labels.json
    .venv/bin/python3 tools/build/stock_labels.py --check  # JSON still matches the firmware?
...
`fmt(char *buf, int value)` (docs/firmware/PARAM_PAGES.md section 7). So rather than
hand-decode eleven formatters and keep the table honest by inspection, this
runs each one on the emulated ColdFire (tools/emu/emu_bringup.py's detour call,
    ...
    if "--check" in argv:
        ...
        if have != got:
            ...
            sys.exit(f"{OUT.relative_to(ROOT)} does not match the firmware -- "
                     f"regenerate with: {sys.executable} tools/build/stock_labels.py")
