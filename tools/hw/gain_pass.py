...
path drops samples in chunks -- do not go back to it; measured 24 Aug).
    ...
    python3 tools/hw/gain_pass.py solo-map NAME [--machine ot|rytm] [--secs 8]
                                                     # solo each track in turn, capture, report
    python3 tools/hw/gain_pass.py set ot 3 --level 100 --vol 64
    python3 tools/hw/gain_pass.py set rytm 2 --level 90
    python3 tools/hw/gain_pass.py analyse FILE.wav
    python3 tools/hw/gain_pass.py start | stop          # Rytm transport

CC map (manual-confirmed, docs/firmware/MIDI.md "Remote CC reference"):
...
def send(port, *args, tries=3):
    for i in range(tries):
        r = subprocess.run(["python3", str(REPO / "tools/hw/ot_midi.py"), "-p", port]
