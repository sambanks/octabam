...
producing it is the real engine, so this asserts exactly that:
  ...
  1. SAME SAMPLE LOOP.   tools/build/cycle_count.py's span for burn_probe equals
...
def build(env_burn):
    ...
    if env_burn:
        ...
        env.pop("BURN", None)
    run([sys.executable, "tools/build/build_bus.py"], env=env)
    import dsp_modmap
...
def burn_fits():
    ...
    engine we did not ship (docs/firmware/CHIP.md, PLAN.md). The four-line engine is now
