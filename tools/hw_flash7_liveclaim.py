...
could not be measured; a by-hand effect select on the stable A01 source can).
    ...
    python3 tools/hw_flash7_liveclaim.py v      # T8 FX2 = SEND set by hand: the send is refused on the master
    python3 tools/hw_flash7_liveclaim.py vii    # T4 FX1 = CHARACTER, RET up: T4 returns nothing, T8 still returns
    python3 tools/hw_flash7_liveclaim.py iii    # T5 FX2 = SEND (reverb removed): repeats return, no reverb (fall-through)

Rig as tools/hw_flash7.py: OT on UM-ONE, MicroBook out -> inputs A/B for the
...
claim = sys.argv[1]
sys.path.insert(0,"tools"); sys.argv=["x"]; import hw_flash7 as h
