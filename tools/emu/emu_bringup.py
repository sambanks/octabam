#!/usr/bin/env python3
"""Tier-0 ColdFire bring-up harness (the remixer's emulator view).
...
real peripherals, no scheduler. Two uses:
  ...
  * as a CLI, `tools/emu/emu_bringup.py [image]` prints the boot outcome + map;
  * as a library, `boot(image)` returns a warm machine and `read_menu_tree`
    ...
    menu entry (docs/firmware/MAINMENU.md) actually resolves.
...
# Root of the MAIN MENU tree (docs/firmware/MAINMENU.md). Address = file offset + BASE.
...
# EFFECT 1 / EFFECT 2 SETUP window openers (docs/firmware/MAINMENU.md §7) — the chooser
...
def boot(image=None, count=BUDGET, on_draw=None):
    ...
    OVERRIDES = {
        ...
        # callers (tools/emu/emu_card.py) add peripheral replies here before boot()
        **EXTRA_OVERRIDES,
    }
    seen = set()
...
def read_menu_tree(uc, desc=MENU_ROOT_DESC, depth=0, _seen=None):
    """Walk the MAIN MENU tree out of booted RAM (docs/firmware/MAINMENU.md).
...
def _open_menu_window(uc):
    ...
    remixer it never was: FX2 is the default view. Found by
...
def _call_page(uc, entry, draws):
    ...
    LCD half the time. Found by calling render_fx2 four times in
...
def render_playback(r, track=4, machine=1):
    ...
    the machine-type table: 0 = STATIC, 1 = FLEX (measured, RTOS_FORK
