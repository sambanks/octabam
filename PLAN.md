...
| Wrapped, NOT flashed | ✅ **`OCTABAMR30` (tag 49), wrapped 18 Aug 2026** — `out/OCTATRACK_OCTABAMR30.bin`, 445,692 bytes, crc32 `0x8d4dae01`, round-trip + checksum ok, `make check` green, differs from R29. `ChonVerb49` ×1, `BongDelay49` ×1, 49 the only tag. = R29 **plus `-VRB`** (`6d2690b`): the delay's `→VERB` send is a **knob again** (p5, default 0, registration follows it — kills the last unconditional registration in the box). Steps measured exact (+6.02/+5.95 dB), `-VRB` 0 = digital silence AND zero dilution of other senders. ⚠️ **No delay→reverb wash out of the box** until `-VRB` comes up. ⚠️ Old projects: see `docs/FLASHING.md`'s new "first load of an existing project" step — old VRBW/MIX stored values load as `-VRB`/IN; re-selecting the effect per track applies fresh defaults |
| On the unit | **`OCTABAMR29`** (tag 48) — flashed 17 Aug, ✅ **ear-passed ("sounds pretty good")**: the v4 RETURN reverb plays right, symmetric with the delay. Previously **`OCTABAMR28`** (tag 47) — **FLASHED 17 Aug 2026**, and ✅ **THE FIVE NEVER-WORKED KNOBS NOW WORK** (Sam: "knobs are working now"): delay WOW/PTCH/FRZE, reverb WIDTH and →DEL — the companion-field fix confirmed on hardware. Also confirmed on the R26–R28 series the same day: all three cross-core defects closed (sweep clean on all tracks × all modes), levels hold across sender counts (1/√N), correlated content does not clip in practice, the PLATE/BIG static gone, BIG does not break up at Sam's levels |
