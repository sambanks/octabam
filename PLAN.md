...
| Wrapped, NOT flashed | ✅ **`OCTABAMR32` (tag 51), wrapped 18 Aug 2026** — `out/OCTATRACK_OCTABAMR32.bin`, 445,728 bytes, crc32 `0x2f5be855`, `make check` green, `ChonVerb51`/`BongDelay51`, 51 the only tag. **Supersedes the unflashed R30 AND R31 — ⚠️ both carry a BROKEN delay IN** (the `-VRB` splice deleted its decode; caught 18 Aug, neither reached hardware). = R31's content **plus the IN fix and the IN/`-VRB` slot swap** (IN bottom-right on BOTH effects: delay IN p4→p5, `-VRB` p5→p4; measured +5.95 dB exact at the new slot, silence at 0). verify_bus grew two host-IN cases (19 total) so a zero-default knob can never again die silently. **Flash R32; discard R30/R31** |
