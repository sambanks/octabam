...
| Wrapped, NOT flashed | ✅ **`OCTABAMR36` (tag 55), wrapped 18 Aug 2026** — 445,772 bytes, crc32 `0x8d531ad7`, supersedes the unflashed R35 (contains it). = R34 + **TAPE retired into global DPTH/RATE** (old TAPE ≡ CLEAN+knobs bit-exactly) + **DRIVE** (p10 = DRV everywhere but GRAIN, loop-stable blend `w + d·(sat(2w)/2 − w)`, default 0 = exact bypass; GRAIN keeps SPRA on the same knob, d pinned 0). satdrv rolled into one `bsr` (the word cost that blocked drive since 4b); cycle_count learned to price bsr callees honestly (worst path 1959, ~9% core-1 margin). All gates bit-identity vs R35. ⚠️ p10 default now 0: a fresh GRAIN part boots fully coherent |
