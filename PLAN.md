...
| Wrapped, NOT flashed | ✅ **`OCTABAMR31` (tag 50), wrapped 18 Aug 2026** — `out/OCTATRACK_OCTABAMR31.bin`, 445,724 bytes, crc32 `0x11b83b82`, round-trip + checksum ok, `make check` green, differs from R30. `ChonVerb50` ×1, `BongDelay50` ×1, 50 the only tag. **Supersedes the unflashed R30** (tag 49, never on hardware — flash R31, discard R30). = R30 **minus ChonVerb's `→DEL`** (`f9c1390`, the vestige twin of VRBD; page-2 slot 11 blank) **plus R30's `-VRB` knob**. The reverb now writes NO bus at all; the only wet-crossing edge is delay→reverb under `-VRB`. Retirement proven inert: verify_bus 17/17 bit-identical vs R30. Payload A **FREE 98** (roomiest since the 8-line engine), r7 $68/$69/$6a/$71 free |
