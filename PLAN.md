...
| Wrapped, NOT flashed | ✅ **`OCTABAMR20` (tag 39), wrapped 17 Aug 2026** — `out/OCTATRACK_OCTABAMR20.bin`, 445,668 bytes, crc32 `0x413eb044`, round-trip ok, `make check` green (`verify_burn` SKIPPED as documented). Carries the page-2 formatter fix and **the whole of BongDelay v3** — the RETURN architecture, MIX→IN, `→VERB` hardwired at max and registered, PING sweeping centred-mono→ping-pong. Names verified inside the packed build: `ChonVerb39` ×1, `BongDelay39` ×1, tag 39 the only one present. ⚠️ **Ships with one known defect: ChonVerb is a phantom client of the delay bus** (−6 dB with a single sender); the fix is 5 words and payload A has FREE 4 — see 3.2. Version `R20` is next-free, tag 39 is the effect-name identity; the two counters are independent and always have been |
