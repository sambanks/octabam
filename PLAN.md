...
| M6e use it | 🟡 in progress (6 Sep) | the recorder-project fixture now exists (`tools/scratch/make_recorder_testproj.py`: track 1 -> machine type 4/PICKUP1, verified by RAM readback); with it, track 1's existing trig stops producing the ordinary `FW_LIVE_NIBBLE` write entirely — confirms machine-type-branched trig dispatch (`ARCHITECTURE.md`), the first real signal PICKUP takes a different path. The actual arm/record code path is still unlocated: every named candidate (`FUN_400977cc`, `FUN_40097168`, the recorder TRIG branch `0x40083544`, `FUN_40005178`, the arm caller `0x40005ff0`, even `0x4000b800`) logged zero calls, `0x4000b800` included on the ORIGINAL project's own successful trig — so that address isn't reached the way `EMU.md` assumed, independent of the recorder question. Best untested guess: a PICKUP machine needs a recorder-buffer assignment (ids 128-135) the way a STATIC track needs a sample slot, and this pass only touched the raw machine-type byte. RTOS_FORK §10 |

Where to resume: **M6e, use it** — the recorder-project fixture exists;
next is locating the real per-step arm/record path (start from a buffer-
assignment field, by analogy with `ot_project.py`'s static-slot field,
rather than more blind `watch_calls` guesses) and Bryan's click question.
